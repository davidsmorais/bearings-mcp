...
**A test file lives in the same directory as the code it tests, named `<target>.test.ts`** — `src/tools/resolveDestination.ts` is tested by `src/tools/resolveDestination.test.ts`, `src/http/cache.ts` by `src/http/cache.test.ts`, and so on (the way `packages/shared` colocates its `*.test.ts`). `packages/server/test/` holds only what has no single target: the committed upstream fixtures (`test/fixtures/`) and cross-cutting integration suites (`test/server.integration.test.ts`), which a colocated test imports from via a relative path. `vitest.config.ts` scans both `src/**/*.test.ts` and `test/**/*.test.ts`; `passWithNoTests` is `false`, so a suite that collected nothing fails. Type-checking test files is a separate pass — `tsconfig.json` builds only `src/` into `dist/` (it excludes `src/**/*.test.ts`), and `tsconfig.test.json` (no emit) covers both `src/` and `test/`. The `typecheck` script runs both configs.
